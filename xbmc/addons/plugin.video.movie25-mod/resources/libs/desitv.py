...
def LISTEPISODES(tvshowname,url):
    ...
    for url,name in match:
        ...
        main.addInfo(name,MainUrl+url,39,'','','') 
        loadedLinks = loadedLinks + 1
        percent = (loadedLinks * 100)/totalLinks
        remaining_display = 'Episodes loaded :: [B]'+str(loadedLinks)+' / '+str(totalLinks)+'[/B].'
        dialogWait.update(percent,'[B]Will load instantly from now on[/B]',remaining_display)
    ...
    for string in match:
        ...
        for url, page in match1:
            main.addInfo(page,MainUrl+url,38,'','','')
    dialogWait.close()
    del dialogWait
    xbmcplugin.setContent(int(sys.argv[1]), 'TV Shows')
    main.VIEWS()
...
def VIDEOLINKS(name, url):
    ...
    link=main.OPENURL(url)
    link=link.replace('\r','').replace('\n','').replace('\t','').replace('&nbsp;','')
    match = re.findall('<blockquote class="postcontent restore ">.+?)</blockquote>',link)
    for e in match:
        print 'E >>>>>>>>>>>>>' + e
        match1=re.findall('<b><Font.+?</Font></b>', e)
        print match1
